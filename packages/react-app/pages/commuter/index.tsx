...
//           >
//             {loading ? "Processing..." : `Pay ${amount} cU$D`}
//           </button>
//         </div>
//       )}
...
//   const userTransactions = data?.paymentMades.filter(
//     (transaction: any) => transaction.payer.toLowerCase() === address?.toLowerCase()
//   );
...
//         onClick={handlePay}
//         disabled={loading}
//         className={`w-1/2 p-3 rounded-md text-white font-semibold ${loading ? "bg-gray-500 cursor-not-allowed" : "bg-green-500 hover:bg-green-600"}`}
//       >
...
//       {/* Last 5 Transactions */}
//       <div className="w-full max-w-md space-y-4 mt-6">
//         <h3 className="text-xl text-gray-200 font-semibold">Last 5 Transactions</h3>
//         {transactionsLoading ? (
//           <p>Loading transactions...</p>
//         ) : error ? (
//           <p>Error loading transactions: {error.message}</p>
...
export default function CommuterPage() {
  ...
  const handlePay = async () => {
    await payUser(recipient, amount);
    setIsProcessingComplete(true); // Hide buttons after payment is complete
  };
